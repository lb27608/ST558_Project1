...
trial<-function(yr,AGEP=TRUE,GASP=FALSE,GRPIP=FALSE,JWAP=FALSE,JWDP=FALSE,JWMNP=FALSE,FER=FALSE,HHL=FALSE,HISPEED=FALSE,JWTRNS=FALSE,SCH=FALSE,SCHL=FALSE,SEX=TRUE,geography='state',geo_code=04){
...
final<-left_join(final,x2,by=join_by(JWAP==name))|>
  select(-JWAP)|>
  mutate(time=ifelse(is.na(time),'N/A (not a worker; worker who worked from home)',time))|>
  rename("JWAP"="time")
}
if(FER){
...
attempt<-trial(2022,FER=TRUE,geography='division',geo_code=8,HHL=TRUE,JWAP=FALSE)
attempt
...
pums_years <- function(yr_list) {
  ...
  for(i:yr_len){
    ...
    yr_tib <- trial(yr,FER=TRUE,geography='division',geo_code=8,HHL=TRUE,JWAP=FALSE) |> mutate(year=yr)

...
summary.census <- function(cens_tibble,num_vars,cat_vars){
  ...
  # If not, include all numeric variables except for PWGTP and year
  if (missing(cat_vars)) {
    cat_vars <- cens.tibble |> select(where(!is.numeric))
  } 
  
  # Subset the tibble to include only the categorical variables   
  cat_tibble <- cens_tibble |> select(all_of(cat_vars))
  
  # Generate counts for each variable
  
  
  return(as.list(mean_t),as.list(sd_t))

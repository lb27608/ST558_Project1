...
Let's say we wanted to dive further into this combination of variables, just for the year 2019. We'll call our API function, get_PUMS_API, for the year 2019.

```{r}
    our_PUMS_data_1<-get_PUMS_API(yr=2019, SCH=TRUE, SEX=FALSE, geography = 'region',geo_code = 01)
```

We can now calculate summary statistics for age and school enrollment.

```{r}
summary.census(cens_tibble=our_PUMS_data_1,num_var="AGEP",cat_var="SCH")
```
